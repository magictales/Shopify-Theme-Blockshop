(function() {
  var Accordion, HorizontalTabs, Main, Popup, VerticalTabs, el,
    ...
    FrameworkCollection.prototype.getGridHtml = function() {
      ...
      _this.request.onload = function() {
        ...
        if (_this.request.status >= 200 && _this.request.status < 300) {
          ...
          _this.spinner.hide();
          _this.grid_container.removeAttr('style');
  ...
  theme.classes.FrameworkFeaturedProduct = (function() {
    ...
    FrameworkFeaturedProduct.prototype.updateVariantOnOptionChange = function() {
      ...
      return _this.option_inputs.on('change', function() {
        var current_options, j, len, options, ref, variant, variant_title;
  ...
  theme.classes.FrameworkProduct = (function() {
    ...
    FrameworkProduct.clearQuickAddForms = function(product_id) {
      if (product_id == null) {
        product_id = '';
      }
      return el('.off-canvas--right-sidebar').find(".product--form[data-product-id*='" + product_id + "']").remove();
    };
  ...
  Accordion = (function() {
    function Accordion() {
      ...
      var _this;
      _this = this;
      _this.rte_container = el('.accordion').closest('.product-page--rte');
      if (_this.rte_container.length) {
        _this.load();
      }
    }

    Accordion.prototype.load = function() {
      var _this, accordion_content, accordion_headings, triggers;
      _this = this;
      _this.rte_container.find('.accordion.headings').each(function(heading) {
        return el(heading).add(el(heading).next('.accordion.content')).wrapAll('<div class="accordion-wrapper">', '</div>');
      });
      _this.rte_container.find('.accordion.headings li').wrapInner('<div class="trigger">', '<div class="bg"></div></div>');
      accordion_headings = _this.rte_container.find('.accordion.headings > li');
      accordion_content = _this.rte_container.find('.accordion.content > li');
      accordion_content.each(function(content, index) {
        var content_el;
        content_el = theme.utils.parseHtml("<div class='content'>" + (el(content).html()) + "</div>");
        return accordion_headings.eq(index).append(content_el);
      });
      accordion_content.remove();
      triggers = _this.rte_container.find('.trigger');
      triggers.on('click', function() {
        var other_panels, panel_height, this_panel;
        el(this).closest('.accordion').find('li').removeClass('active');
        el(this).parent().addClass("active");
        this_panel = el(this).siblings('.content');
        panel_height = theme.utils.getHiddenElHeight(this_panel);
        this_panel.css('height', panel_height + "px");
        other_panels = el(this).closest('.accordion-wrapper').find('.content').not(this_panel);
        return other_panels.removeAttr('style');
      });
      return triggers.first().trigger('click');
    };
  ...
  theme.classes.Cart = (function(superClass) {
    ...
    Cart.prototype.updateTotalsComplete = function(count) {
      return el('.header--cart-number').attr('data-item-count', count);
    };
  ...
  theme.classes.Header = (function() {
    ...
    Header.prototype.moveYMenu = function() {
      ...
      _this = this;
      y_menu = el('.y-menu');
      return el('.mobile-nav--menu').empty().append(y_menu);
    };

    Header.prototype.moveLocalizationForm = function() {
      var _this, localization_form;
      _this = this;
      localization_form = el('.header--localization-for-off-canvas form');
      if (localization_form.length) {
        return el('.mobile-nav--localization').empty().append(localization_form);
      }
    };
  ...
  HorizontalTabs = (function() {
    function HorizontalTabs() {
      ...
      var _this;
      _this = this;
      _this.rte_container = el('.tabs-horizontal').closest('.product-page--rte');
      if (_this.rte_container.length) {
        _this.load();
      }
    }

    HorizontalTabs.prototype.load = function() {
      var _this, tab_content;
      _this = this;
      _this.rte_container.find('.tabs-horizontal').each(function(heading) {
        return el(heading).add(el(heading).next('.tabs.content')).wrapAll('<div class="tabs-wrapper horizontal">', '</div>');
      });
      _this.rte_container.find('.tabs-horizontal li').wrapInner('<div class="trigger" tabindex="0">', '<div class="bg"></div></div>');
      tab_content = _this.rte_container.find('.tabs.content > li');
      tab_content.first().addClass('active');
      return _this.rte_container.find('.tabs-wrapper.horizontal').each(function(wrapper) {
        var tab_headings;
        tab_headings = el(wrapper).find('.headings > li');
        tab_headings.first().addClass('active');
        return tab_headings.on('click keydown', function(event) {
          if (event.type === 'keydown' && event.key !== 'Enter') {
            return;
          }
          ...
          return tab_content.eq(el(this).index()).addClass('active');
        });
      });
    };
  ...
  Popup = (function() {
    function Popup(clazz) {
      var checkContentOverflow, closeDialog, eventListeners, fillPopupDialog, getCustomClasses, getImage, getNewsletter, getSocialIcons, getWrapper, hideMask, ieCenter, isExpired, mask, maskIsActive, modal, modal_clazz, openDialog, passesExpiration, resetExpiration, showMask, storeExpiration;
      this.clazz = clazz;
      modal = null;
      modal_clazz = this.clazz;
      mask = el('.popup-modal-mask');
      storeExpiration = function() {
        ...
        try {
          return localStorage[theme.local_storage.popup] = JSON.stringify(object);
        } catch (error1) {
          e = error1;
          return false;
        }
      ...
      resetExpiration = function() {
        localStorage.removeItem(theme.local_storage.popup);
        return storeExpiration();
      };
      isExpired = function() {
        var expires, now, object;
        object = JSON.parse(localStorage[theme.local_storage.popup]);
      ...
      passesExpiration = function() {
        ...
        if (Storage === "undefined" || localStorage === null || theme.popup.test_mode) {
          passed = true;
        } else if (typeof localStorage[theme.local_storage.popup] === "undefined") {
          passed = true;
          storeExpiration();
        } else {
          passed = isExpired();
      ...
      maskIsActive = function() {
        return el('.popup-modal').isVisible();
      };
      showMask = function() {
        mask.show();
        return el('body').addClass("modal-on");
      };
      hideMask = function() {
        mask.hide();
        return el('body').removeClass("modal-on");
      };
      getImage = function() {
        var popup_image, popup_image_container;
        if (theme.popup.show_image_enabled === false) {
          return "";
        }
        if (theme.popup.image_link.length > 0) {
          return theme.utils.parseHtml("<div class='popup-image'> <a href='" + theme.popup.image_link + "'>" + theme.popup.show_image_url + "</a> </div>");
        } else {
          popup_image_container = theme.utils.parseHtml('<div class="popup-image">');
          popup_image = theme.utils.parseHtml(theme.popup.show_image_url);
          return popup_image_container.append(popup_image);
        }
      };
      getNewsletter = function() {
        ...
        subscribe_module = theme.utils.parseHtml("<div id='subscribe_module'></div>");
        subscribe_module = subscribe_module.append(el(".popup--root"));
      ...
      getSocialIcons = function() {
        ...
        return el(".footer--social-icons").clone();
      };
      ...
      getWrapper = function() {
        return theme.utils.parseHtml("<dialog class='" + modal_clazz + " popup-modal " + (getCustomClasses()) + "' />");
      };
      fillPopupDialog = function() {
        var dialog, getInnerContent, render, request;
        dialog = {
          body: el(),
          image: getImage(),
        ...
        getInnerContent = function() {
          ...
          inner_div = theme.utils.parseHtml("<div class='inner'/>");
          return inner_div.append(dialog.body).append(dialog.newsletter).append(dialog.social_icons);
        };
        render = function() {
          el('body').append(dialog.wrapper.append(dialog.image).append(getInnerContent()));
          return openDialog();
        };
        if (theme.popup.page_content.length > 0) {
          ...
          request.onload = function() {
            var body_html;
            if (request.status >= 200 && request.status < 300) {
              body_html = JSON.parse(request.response).page.body_html;
              dialog.body = theme.utils.parseHtml("<div class='page-contents'>" + body_html + "</div>");
              return render();
            }
          };
          request.onerror = function() {
            return console.log(request.statusText + ": popup.json request failed!");
          };
          request.open("GET", "/pages/" + theme.popup.page_content + ".json");
          return request.send();
        } else {
          return render();
        }
      ...
      checkContentOverflow = function() {
        return setTimeout(function() {
          if (modal.length > 0 && el('.popup-modal-mask').isVisible()) {
            return theme.utils.imagesLoaded(modal, function() {
              var dialog_height;
              dialog_height = el("dialog[class*='" + modal_clazz + "']:last-of-type").outerHeight();
              if (dialog_height >= window.innerHeight) {
                return el('body').addClass('modal-unfix');
              } else {
                return el('body').removeClass('modal-unfix');
              }
            });
          }
        }, 0);
      ...
      ieCenter = function() {
        modal.css('margin-top', (-(modal.outerHeight() * 0.5)) + "px");
        return modal.css('margin-left', (-(modal.outerWidth() * 0.5)) + "px");
      };
      openDialog = function() {
        modal = el("dialog[class*='" + modal_clazz + "']");
        eventListeners();
      ...
      eventListeners = function() {
        ...
        el('.popup-modal').on('click', function(event) {
          event.stopPropagation();
          if (event.target === this) {
            return closeDialog();
          }
        });
        el('.popup-modal-close').on('click', closeDialog);
        ...
        return el('dialog.popup-modal #contact_form').on('submit', function(event) {
          var error_el, form;
          form = el(this);
          modal = form.closest('.popup-modal');
          modal.find('.error, .success').remove();
          event.preventDefault();
          if (modal.find('input[type="email"]').val().length === 0) {
            error_el = theme.utils.parseHtml("<p class='error'>" + theme.translations.newsletter_email_blank + "</p>");
            modal.find('.inner').prepend(error_el);
            return false;
          } else {
            form.submit();
          }
      ...
      if (theme.popup.enabled && passesExpiration() && (theme.popup.page_content.length > 0 || theme.popup.newsletter_enabled || theme.popup.show_image_enabled || theme.popup.social_icons_enabled)) {
        setTimeout(function() {
          if (!(maskIsActive() === true || el('.popup-modal-mask').length < 1)) {
  ...
  VerticalTabs = (function() {
    function VerticalTabs() {
      ...
      var _this;
      _this = this;
      _this.rte_container = el('.tabs-vertical').closest('.product-page--rte');
      if (_this.rte_container.length) {
        _this.load();
      }
    }

    VerticalTabs.prototype.load = function() {
      var _this, tab_content;
      _this = this;
      _this.rte_container.css('display', 'flex');
      _this.rte_container.find('.tabs-vertical').each(function(heading) {
        return el(heading).add(el(heading).next('.tabs.content')).wrapAll('<div class="tabs-wrapper vertical">', '</div>');
      });
      _this.rte_container.find('.tabs-vertical li').wrapInner('<div class="trigger">', '<div class="bg"></div></div>');
      tab_content = _this.rte_container.find('.tabs.content > li');
      tab_content.first().addClass('active');
      return _this.rte_container.find('.tabs-wrapper.vertical').each(function(wrapper) {
        var tab_headings, tab_wrapper;
        tab_wrapper = el(wrapper);
        tab_headings = tab_wrapper.find('.headings > li');
        tab_headings.first().addClass('active');
        return tab_headings.on('click', function() {
          tab_headings.removeClass('active');
          tab_content.removeClass('active');
          el(this).addClass('active');
          tab_content.eq(el(this).index()).addClass('active');
          if (theme.shop.scroll_to_active_item) {
            return window.scrollTo({
              top: tab_wrapper.offset().top - 50,
              left: 0,
              'behavior': 'smooth'
            });
          }
  ...
  theme.classes.XMenu = (function(superClass) {
    ...
    XMenu.prototype.isHeaderTransparent = function() {
      var _this;
      _this = this;
      if (el('.header--root').data('header-menu-transparent')) {
        return true;
      } else {
        return false;
      }
    ...
    XMenu.prototype.showHeader = function() {
      var _this;
      _this = this;
      return _this.header.attr('data-transition', 'forwards');
    };
    ...
    XMenu.prototype.hideHeader = function() {
      ...
      _this.header.attr('data-transition', 'backwards');
      return _this.slideUp();
    };
  ...
  theme.classes.Utils = (function(superClass) {
    ...
    Utils.prototype.transparentMenuIsEnabled = function() {
      return el('body').find("[data-header-menu-transparent=true]").length > 0;
    };
  ...
  new (Main = (function() {
    function Main() {
      ...
      var _this;
      _this = this;
      _this.on_template_page = el('body').hasClass('template--page');
      _this.load();
    }
    ...
    Main.prototype.configureColumns = function() {
      var page_content;
      page_content = el('.template-page--main [data-item="rte-content"]');
      if (page_content.find('.left-side-column').length || page_content.find('.right-side-column').length) {
        if (page_content.find('.left-side-column').length && page_content.find('.right-side-column').length) {
          ...
          el('.left-side-column').addClass('with-2-sidebars');
          el('.right-side-column').addClass('with-2-sidebars');
        } else {
          page_content.wrapInner('<div class="main-column">', '</div>');
        }
        page_content.prepend(el('.left-side-column'));
        return page_content.append(el('.right-side-column'));
      }
    };
